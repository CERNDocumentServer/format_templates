...
def format_element(bfo, source_format='%Y-%m-%d', date_format='%d %B %Y'):
    """
    ...
    @param source_format the format of the date in the metadata
    """
    date = bfo.field('269__c')
    if date_format != '':
        try:
            date_time = time.strptime(date, source_format)
